...
async function generateTestJson() {
    try {
        ...
        const nominaId = 3; // Change this to a valid one if needed

        // Fetch data similar to service
        const nomina = await connection.query('SELECT * FROM nominas WHERE id = $1', [nominaId]);
        if (!nomina.length) {
            console.log('Nomina not found');
            return;
        }
        const n = nomina[0];
    ...
    `, [nominaId]);
        ...
        const heFestivas = details.filter(d => d.subtipo === 'HORA_EXTRA_FESTIVA');
        ...
        const json = {
            ...
            period: {
                ...
                worked_time: Math.round(n.dias_pagados * 8),
            ...
            worker: {
                ...
                payroll_type_document_identification_id: parseInt(empleadoDetallado?.tipo_documento_id || '1'),
        ...
        fs.writeFileSync('test_nomina_dian.json', JSON.stringify(json, null, 4));
        console.log('JSON generated in test_nomina_dian.json');
