...
import type { CreateTipoPersonaDto } from '../types/tipo-persona';
...
const formData = ref<CreateTipoPersonaDto>({
  codigo: 0,
  nombre: '',
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await tipoPersonaService.create(formData.value);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al crear el tipo de persona';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Nuevo Tipo de Persona</h1>
            <p class="form-subtitle">Complete la información del tipo de persona</p>
          </div>
        </div>
      </div>
