...
import type { UpdateTipoPersonaDto } from '../types/tipo-persona';
...
const formData = ref<UpdateTipoPersonaDto>({
  codigo: 0,
  nombre: '',
  empresa_id: authStore.user?.empresaId || 1
});

const loadTipoPersona = async () => {
  try {
    loadingData.value = true;
    const tipoPersona = await tipoPersonaService.getByCodigo(codigo.value);
    formData.value = {
      codigo: tipoPersona.codigo,
      nombre: tipoPersona.nombre,
      empresa_id: tipoPersona.empresa_id
    };
  } catch (error) {
    console.error('Error al cargar tipo de persona:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los datos del tipo de persona'
    });
    router.push('/tipo-persona');
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await tipoPersonaService.update(codigo.value, dataToUpdate);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al actualizar el tipo de persona';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Editar Tipo de Persona</h1>
            <p class="form-subtitle">Modifique la información del tipo de persona</p>
          </div>
        </div>
      </div>
