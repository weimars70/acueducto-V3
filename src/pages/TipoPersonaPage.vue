...
import type { TipoPersona } from '../types/tipo-persona';

const $q = useQuasar();
const router = useRouter();
const tipoPersonas = ref<TipoPersona[]>([]);
...
const loadData = async () => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los tipos de persona'
    });
    tipoPersonas.value = [];
...
const handleNew = () => {
  router.push('/tipo-persona/new');
};

const handleEdit = (tipoPersona: TipoPersona) => {
  router.push(`/tipo-persona/edit/${tipoPersona.codigo}`);
};

const handleDelete = async (tipoPersona: TipoPersona) => {
  $q.dialog({
    title: 'Confirmar',
    ...
    try {
      await tipoPersonaService.delete(tipoPersona.codigo);
      $q.notify({
        type: 'positive',
      ...
      $q.notify({
        type: 'negative',
        message: 'Error al eliminar el tipo de persona'
      });
    }
  });
};
...
const paginatedTipoPersonas = computed(() => {
  if (viewMode.value === 'grid') {
    const start = (pagination.value.page - 1) * pagination.value.rowsPerPage;
    const end = start + pagination.value.rowsPerPage;
    return tipoPersonas.value.slice(start, end);
  }
  return tipoPersonas.value;
});
...
<template>
  <q-page class="tipo-persona-page">
    <div class="page-container">
      ...
      <div class="page-header">
        <div class="header-content">
          <div class="title-section">
            ...
            <div>
              <h1 class="page-title">Tipo Persona</h1>
              <p class="page-subtitle">Gestión de tipos de persona</p>
            </div>
          </div>
          <q-btn
            unelevated
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center q-gutter-md q-py-xl">
              ...
              <div class="text-center">
                <div class="text-h6 text-grey-6">No hay tipos de persona registrados</div>
      ...
      <div v-else class="grid-container">
        ...
        <div v-if="!loading && tipoPersonas.length === 0" class="empty-state">
          <q-icon name="person" size="96px" color="grey-4" />
          <h2 class="text-h5 text-grey-6 q-mt-md">No hay tipos de persona registrados</h2>
        ...
        <div v-else class="cards-grid">
          <q-card
            v-for="tipoPersona in paginatedTipoPersonas"
            :key="tipoPersona.codigo"
            class="tipo-card"
            flat
            bordered
          >
            <q-card-section class="card-header">
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div class="q-ml-md">
                    <div class="card-title">{{ tipoPersona.nombre }}</div>
                    <div class="card-code">Código: {{ tipoPersona.codigo }}</div>
                  </div>
                </div>
              </div>
            </q-card-section>
            ...
            <q-card-actions class="card-actions">
              ...
              <q-btn
                ...
                label="Eliminar"
                color="negative"
                @click="handleDelete(tipoPersona)"
              />
            </q-card-actions>
          </q-card>
        </div>
        ...
        <div v-if="tipoPersonas.length > 0" class="grid-pagination">
          <q-btn
            flat
            icon="chevron_left"
            label="Anterior"
