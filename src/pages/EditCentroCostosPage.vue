...
import type { UpdateCentroCostosDto } from '../types/centro-costos';
...
const centroCostosId = ref<number>(parseInt(route.params.id as string));

const formData = ref<UpdateCentroCostosDto>({
  nombre: '',
  usuario: authStore.user?.email || ''
});

const loadCentroCostos = async () => {
  try {
    loadingData.value = true;
    const centroCostos = await centroCostosService.getById(centroCostosId.value);
    formData.value = {
    ...
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los datos del centro de costos'
    });
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await centroCostosService.update(centroCostosId.value, formData.value);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al actualizar el centro de costos';
    $q.notify({
      type: 'negative',
      message
    });
    ...
    <div class="form-container">
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Editar Centro de Costos</h1>
            <p class="form-subtitle">Modifique la información del centro de costos</p>
          </div>
        </div>
      </div>
      ...
      <q-card v-if="loadingData" flat class="form-card">
        <q-card-section class="loading-section">
          ...
          <div class="loading-text">Cargando datos del centro de costos...</div>
        </q-card-section>
      </q-card>
