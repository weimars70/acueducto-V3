...
import type { NotaCredito } from '../types/nota-credito';
...
const loadData = async () => {
  try {
    loading.value = true;
    // Aquí deberías pasar el empresaId adecuado
    const data = await notaCreditoService.getAll();
    notasCredito.value = data;
...
const handleNew = () => {
  router.push('/notas-credito/nuevo');
};

const handleEdit = (notaCredito: NotaCredito) => {
  router.push(`/notas-credito/editar/${notaCredito.codigo}/${notaCredito.empresaId}`);
};

const handleDelete = async (notaCredito: NotaCredito) => {
  $q.dialog({
    title: 'Confirmar',
    message: `¿Está seguro de eliminar la nota de crédito #${notaCredito.codigo}?`,
    cancel: true,
    persistent: true
  }).onOk(async () => {
    try {
      await notaCreditoService.delete(notaCredito.codigo, notaCredito.empresaId);
      $q.notify({
        type: 'positive',
...
const filteredNotasCredito = computed(() => {
  if (!filter.value) return notasCredito.value;

  const searchTerm = filter.value.toLowerCase();
  return notasCredito.value.filter(nc =>
    nc.codigo?.toString().includes(searchTerm) ||
    nc.clienteNombre?.toLowerCase().includes(searchTerm) ||
    nc.instalacionCodigo?.toString().includes(searchTerm)
  );
});

const paginatedNotasCredito = computed(() => {
  const start = (pagination.value.page - 1) * pagination.value.rowsPerPage;
  const end = start + pagination.value.rowsPerPage;
  return filteredNotasCredito.value.slice(start, end);
});

const totalPages = computed(() => {
  return Math.ceil(filteredNotasCredito.value.length / pagination.value.rowsPerPage);
});
...
<template>
  <q-page class="notas-credito-page">
    <div class="page-container">
        ...
        <div class="header-content">
          <div class="title-section">
            ...
            <div>
              ...
              <p class="page-subtitle">Gestión de notas de crédito</p>
            </div>
          </div>
          <q-btn
            unelevated
      ...
      <q-card v-if="viewMode === 'table'" flat class="content-card">
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center q-gutter-md q-py-xl">
              ...
              <div class="text-center">
                <div class="text-h6 text-grey-6">No hay notas de crédito registradas</div>
                <div class="text-caption text-grey-5">Comienza agregando una nueva nota de crédito</div>
              </div>
            </div>
          </template>
        </q-table>
        ...
        <!-- Paginación -->
        <div v-if="filteredNotasCredito.length > 0" class="table-pagination">
          <q-btn
            flat
            icon="chevron_left"
            label="Anterior"
      ...
      <div v-else class="grid-container">
        ...
        <div v-if="!loading && filteredNotasCredito.length === 0" class="empty-state">
          <q-icon name="receipt_long" size="96px" color="grey-4" />
          <h2 class="text-h5 text-grey-6 q-mt-md">No hay notas de crédito registradas</h2>
          <p class="text-grey-5">Comienza agregando una nueva nota de crédito</p>
          <q-btn
            unelevated
            color="primary"
            icon="add"
        ...
        <div v-else class="cards-grid">
          <q-card
            v-for="nc in paginatedNotasCredito"
            :key="`${nc.codigo}-${nc.empresaId}`"
            class="nota-card"
            flat
            bordered
          >
            <q-card-section class="card-header">
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div class="q-ml-md">
                    <div class="card-title">Nota #{{ nc.codigo }}</div>
                    <div class="card-code">{{ nc.clienteNombre || 'Sin cliente' }}</div>
                  </div>
                </div>
                <q-chip
                  :color="(nc.disponible || 0) > 0 ? 'positive' : 'grey'"
                  text-color="white"
                  size="sm"
                  dense
                >
                  {{ (nc.disponible || 0) > 0 ? 'Disponible' : 'Agotada' }}
                </q-chip>
              </div>
            </q-card-section>
            ...
            <q-card-section class="card-body">
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value">{{ nc.instalacionCodigo }}</div>
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value">{{ formatDate(nc.fecha) }}</div>
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value">{{ formatCurrency(nc.valor) }}</div>
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value text-positive">{{ formatCurrency(nc.disponible) }}</div>
                </div>
              </div>

              <div v-if="nc.observacion" class="info-row">
                <q-icon name="notes" size="20px" color="grey-7" />
                <div class="info-content">
                  <div class="info-label">Observación</div>
                  <div class="info-value">{{ nc.observacion }}</div>
                </div>
              </div>
            </q-card-section>
        ...
        <div v-if="filteredNotasCredito.length > 0" class="grid-pagination">
          <q-btn
            flat
            icon="chevron_left"
            label="Anterior"
