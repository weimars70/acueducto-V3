...
import type { CreateTipoRegimenDto } from '../types/tipo-regimen';
...
const formData = ref<CreateTipoRegimenDto>({
  codigo: 0,
  nombre: '',
  code: '',
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await tipoRegimenService.create(formData.value);
    $q.notify({
      type: 'positive',
      message: 'Tipo de regimen creado exitosamente'
    });
    router.push('/tipo-regimen');
  } catch (error: any) {
    console.error('Error al crear tipo de regimen:', error);
    const message = error?.response?.data?.message || error?.message || 'Error al crear el tipo de regimen';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Nuevo Tipo de Régimen</h1>
            <p class="form-subtitle">Complete la información del tipo de régimen</p>
          </div>
        </div>
      </div>
      ...
      <q-card flat class="form-card">
        <q-form @submit="handleSubmit" class="form-content">
          ...
          <div class="form-actions">
            ...
            <q-btn unelevated label="Guardar" type="submit" color="primary" class="action-btn save-btn" :loading="loading" icon="save" />
          </div>
        </q-form>
      </q-card>
    </div>
