...
const loadData = async () => {
  try {
    ...
    // Load item
    const item = await itemsService.getById(parseInt(id), empresaId);
    
    // Find group object
    const selectedGroup = grupos.value.find(g => g.id === item.grupo) || null;
    ...
    $q.notify({ type: 'negative', message: 'Error cargando datos del item' });
    router.push('/items');
  } finally {
    loading.value = false;
  }
...
const handleSubmit = async () => {
  try {
    ...
    await itemsService.update(parseInt(id), {
        ...
        <div class="header-content">
          <div class="title-section">
            ...
            <div>
              ...
              <p class="page-subtitle">Modificar item existente</p>
            </div>
          </div>
        </div>
      </div>
