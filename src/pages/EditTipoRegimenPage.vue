...
import type { UpdateTipoRegimenDto } from '../types/tipo-regimen';
...
const formData = ref<UpdateTipoRegimenDto>({
  codigo: 0,
  nombre: '',
  code: '',
...
const loadTipoRegimen = async () => {
  try {
    ...
    const tipoRegimen = await tipoRegimenService.getByCodigo(codigo.value);
    formData.value = {
    ...
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los datos del tipo de regimen'
    });
    router.push('/tipo-regimen');
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await tipoRegimenService.update(codigo.value, dataToUpdate);
    $q.notify({
      type: 'positive',
      message: 'Tipo de regimen actualizado exitosamente'
    });
    router.push('/tipo-regimen');
  } catch (error: any) {
    console.error('Error al actualizar tipo de regimen:', error);
    const message = error?.response?.data?.message || error?.message || 'Error al actualizar el tipo de regimen';
    $q.notify({
      type: 'negative',
      message
    });
    ...
    <div class="form-container">
      <div class="form-header">
        ...
        <div class="header-content">
          <q-icon name="business" size="40px" color="primary" />
          <div class="header-text">
            <h1 class="form-title">Editar Tipo de Régimen</h1>
            <p class="form-subtitle">Modifique la información del tipo de régimen</p>
          </div>
        </div>
      </div>
