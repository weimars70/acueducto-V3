...
onMounted(async () => {
    try {
        ...
        const sector = await sectoresService.getById(id);
        if (sector) {
            formData.value.nombre = sector.nombre;
        }
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'Error cargando sector' });
...
const handleSubmit = async () => {
    ...
    try {
        loading.value = true;
        await sectoresService.update(parseInt(props.id), formData.value);
        $q.notify({ type: 'positive', message: 'Sector actualizado correctamente' });
        router.push('/sectores');
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'No se pudo actualizar el sector' });
    } finally {
        loading.value = false;
    }
};
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-8 col-lg-6">
        <div class="row items-center q-mb-lg">
             ...
             <div>
                <div class="text-h5 text-weight-bold text-primary">Editar Sector</div>
             </div>
        </div>
        ...
        <q-card class="shadow-4 rounded-xl bg-white overflow-hidden">
          <q-card-section class="q-pa-xl">
            <q-form @submit="handleSubmit" class="q-gutter-md">
                <label class="text-weight-medium text-grey-9 q-mb-xs block">Nombre del Sector</label>
                <q-input
                    outlined
                    dense
