...
import type { CreateNotaCreditoDto } from '../types/nota-credito';
...
const formData = ref<CreateNotaCreditoDto>({
...
onMounted(async () => {
  try {
    const conceptos = await notaConceptoService.getAll();
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await notaCreditoService.create(payload);
    $q.notify({ type: 'positive', message: 'Nota de crédito creada exitosamente' });
    router.push('/notas-credito');
  } catch (error) {
    console.error(error);
    $q.notify({ type: 'negative', message: 'Error al crear la nota de crédito' });
  } finally {
    loading.value = false;
  }
};
...
<template>
  <q-page class="new-nota-credito-page">
            ...
            <div class="header-content">
              <div class="title-section">
                ...
                <div>
                  ...
                  <p class="page-subtitle">Complete la información para crear una nueva nota de crédito</p>
                </div>
              </div>
            </div>
          </div>
