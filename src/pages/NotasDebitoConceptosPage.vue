...
import type { NotaDebitoConcepto } from '../types/nota-debito-concepto';

const $q = useQuasar();
const router = useRouter();

const conceptos = ref<NotaDebitoConcepto[]>([]);
...
const loadConceptos = async () => {
  try {
    loading.value = true;
    conceptos.value = await notaDebitoConceptoService.getAll();
  } catch (error) {
    console.error('Error loading conceptos:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar conceptos de notas débito'
    });
  } finally {
    loading.value = false;
  }
...
const handleEdit = (row: NotaDebitoConcepto) => {
  router.push(`/notas-debito-conceptos/editar/${row.codigo}`);
};

const handleDelete = (row: NotaDebitoConcepto) => {
  $q.dialog({
    ...
    try {
      await notaDebitoConceptoService.delete(row.codigo);
      $q.notify({
        type: 'positive',
  ...
  <q-page class="q-pa-lg bg-grey-1">
    <div class="row items-center justify-between q-mb-lg">
      <div class="row items-center q-gutter-sm">
        ...
        <div>
          <div class="text-h5 text-weight-bold text-primary">Conceptos de Notas Débito</div>
          <div class="text-caption text-grey-7">Gestione los conceptos disponibles para sus notas débito</div>
        </div>
      </div>
      <q-btn
        color="primary"
        ...
        to="/notas-debito-conceptos/nuevo"
        unelevated
