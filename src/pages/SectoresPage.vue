...
const filteredSectores = computed(() => {
  return sectores.value.filter(row => {
    return Object.keys(columnFilters.value).every(key => {
      if (!columnFilters.value[key]) return true;
      const val = String(row[key as keyof Sector] || '').toLowerCase();
...
const loadSectores = async () => {
  try {
    loading.value = true;
    sectores.value = await sectoresService.getAll();
  } catch (error) {
...
const handleEdit = (row: Sector) => {
  router.push(`/sectores/edit/${row.codigo}`);
};

const handleDelete = (row: Sector) => {
  $q.dialog({
    title: 'Confirmar Eliminación',
    message: `¿Está seguro de eliminar el sector "${row.nombre}"?`,
    cancel: true,
    persistent: true,
    ok: { label: 'Eliminar', color: 'negative' }
  }).onOk(async () => {
    try {
      await sectoresService.delete(row.codigo);
      $q.notify({ type: 'positive', message: 'Sector eliminado exitosamente' });
      loadSectores();
    } catch (error) {
       console.error(error);
      $q.notify({ type: 'negative', message: 'Error al eliminar el sector' });
    }
  });
};
...
const handleExportExcel = () => {
  try {
    ...
    exportToExcel(filteredSectores.value, exportColumns, 'sectores');
...
const handleExportPDF = () => {
  try {
    ...
    exportToPDF(filteredSectores.value, exportColumns, 'sectores', 'Listado de Sectores');
  ...
  <q-page class="q-pa-lg bg-grey-1">
    <div class="row items-center justify-between q-mb-lg">
      <div class="row items-center">
        ...
        <div>
          <div class="text-h5 text-weight-bold text-grey-9">Sectores</div>
          <div class="text-caption text-grey-7">Gestión de sectores</div>
        </div>
      </div>
