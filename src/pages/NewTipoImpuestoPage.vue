...
import type { CreateTipoImpuestoDto } from '../types/tipo-impuesto';
...
const formData = ref<CreateTipoImpuestoDto>({
  codigo: 0,
  nombre: '',
  code: '',
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await tipoImpuestoService.create(formData.value);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al crear el tipo de impuesto';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Nuevo Tipo de Impuesto</h1>
            <p class="form-subtitle">Complete la información del tipo de impuesto</p>
          </div>
        </div>
      </div>
