...
const handleExportExcel = () => {
  try {
    ...
    exportToExcel(dataToExport, exportColumns, 'cuotas-conexion');
...
const handleExportPDF = () => {
  try {
    ...
    exportToPDF(dataToExport, exportColumns, 'cuotas-conexion', 'Listado de Cuotas de Conexión');
...
const loadCuotas = async () => {
  try {
    loading.value = true;
    cuotas.value = await diferidosService.getCuotasConexion();
  } catch (error) {
    console.error('Error loading cuotas conexion:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar listado de cuotas conexión'
    });
  } finally {
    loading.value = false;
  }
    ...
    <div class="row items-center justify-between q-mb-lg">
      <div class="row items-center">
        <div class="q-mr-md bg-white q-pa-sm rounded-borders shadow-1">
            <q-icon name="handshake" color="primary" size="md" />
        </div>
        <div>
          <div class="text-h5 text-weight-bold text-grey-9">Cuotas Conexión</div>
          <div class="text-caption text-grey-7">Listado de cuotas diferidas de conexión</div>
        </div>
      </div>
