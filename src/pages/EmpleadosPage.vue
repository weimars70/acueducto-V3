...
const columns = [
  ...
  { name: 'nombre_completo', label: 'Nombre Completo', field: 'nombre_completo', align: 'left' as const, sortable: true },
  { name: 'cargo', label: 'Cargo', field: 'cargo', align: 'left' as const, sortable: true },
  { name: 'salario_mensual', label: 'Salario', field: 'salario_mensual', align: 'right' as const, sortable: true },
  { name: 'fecha_ingreso', label: 'Fecha Ingreso', field: 'fecha_ingreso', align: 'center' as const, sortable: true },
...
const loadData = async () => {
  try {
    ...
    const response = await empleadoService.getEmpleados({
      ...
      nombre_completo: filter.value || undefined
    });
...
const handleNew = () => {
  router.push('/empleados/new');
};

const handleEdit = (empleado: Empleado) => {
  router.push(`/empleados/edit/${empleado.id}`);
};

const handleDelete = async (empleado: Empleado) => {
  $q.dialog({
    title: 'Confirmar',
    message: `¿Está seguro de eliminar al empleado ${empleado.nombre_completo}?`,
    cancel: true,
    persistent: true
  }).onOk(async () => {
    try {
...
const paginatedEmpleados = computed(() => {
  if (viewMode.value === 'grid') {
    ...
    return empleados.value.slice(start, end);
  }
...
const handleExportExcel = () => {
  try {
    const exportColumns = [
      ...
      { field: 'salario_mensual', label: 'Salario' },
      { field: 'fecha_ingreso', label: 'Fecha Ingreso' }
    ];
    exportToExcel(empleados.value, exportColumns, 'empleados');
...
const handleExportPDF = () => {
  try {
    ...
    exportToPDF(empleados.value, exportColumns, 'empleados', 'Listado de Empleados');
    ...
    <div class="page-container">
        ...
        <div class="header-content">
          <div class="title-section">
            ...
            <div>
              ...
              <p class="page-subtitle">Gestión de empleados de nómina</p>
            </div>
          </div>
          <div class="row q-gutter-sm">
            ...
            <q-btn unelevated color="primary" icon="add" label="Nuevo Empleado" class="action-btn" @click="handleNew" />
          </div>
        </div>
      </div>
      ...
      <q-card v-if="viewMode === 'table'" flat class="content-card">
        <q-table
          ...
          class="modern-table"
        >
          <template v-slot:body-cell-cedula="props">
            <q-td :props="props">
              <span class="text-weight-bold text-primary">{{ props.row.cedula }}</span>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-salario_mensual="props">
            <q-td :props="props">
              <span class="text-weight-bold">{{ formatCurrency(props.row.salario_mensual) }}</span>
              <q-chip
                v-if="props.row.auxilio_transporte"
                size="sm"
                color="info"
                text-color="white"
                dense
          ...
          <template v-slot:body-cell-fecha_ingreso="props">
            <q-td :props="props">
              {{ formatDate(props.row.fecha_ingreso) }}
            </q-td>
          </template>
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center q-gutter-md q-py-xl">
              ...
              <div class="text-center">
                <div class="text-h6 text-grey-6">No hay empleados registrados</div>
                <div class="text-caption text-grey-5">Comienza agregando un nuevo empleado</div>
              </div>
            </div>
          </template>
        </q-table>
      ...
      <div v-else class="grid-container">
        ...
        <div v-if="!loading && empleados.length === 0" class="empty-state">
          ...
          <h2 class="text-h5 text-grey-6 q-mt-md">No hay empleados registrados</h2>
          <p class="text-grey-5">Comienza agregando un nuevo empleado</p>
          <q-btn
            unelevated
            color="primary"
            icon="add"
        ...
        <div v-else class="cards-grid">
          ...
          >
            <q-card-section class="card-header">
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div class="q-ml-md">
                    <div class="card-title">{{ empleado.nombre_completo }}</div>
                    <div class="card-code">{{ empleado.cedula }}</div>
                  </div>
                </div>
                <q-chip
                  :color="empleado.activo ? 'positive' : 'negative'"
                  text-color="white"
                  size="sm"
                  dense
                >
            ...
            <q-card-section class="card-body">
              <div class="info-row">
                <q-icon name="work" size="20px" color="grey-7" />
                <div class="info-content">
                  <div class="info-label">Cargo</div>
                  <div class="info-value">{{ empleado.cargo || 'N/A' }}</div>
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value">{{ formatCurrency(empleado.salario_mensual) }}</div>
                </div>
              </div>
              ...
              <div class="info-row">
                ...
                <div class="info-content">
                  ...
                  <div class="info-value">{{ formatDate(empleado.fecha_ingreso) }}</div>
                </div>
              </div>

              <div v-if="empleado.auxilio_transporte" class="info-row">
                <q-icon name="directions_bus" size="20px" color="blue-7" />
                <div class="info-content">
                  <div class="info-label">Auxilio Transporte</div>
                  <div class="info-value text-blue-7">Sí</div>
                </div>
              </div>
            </q-card-section>
        ...
        <div v-if="empleados.length > 0" class="grid-pagination">
          <q-btn
            flat
            icon="chevron_left"
            label="Anterior"
