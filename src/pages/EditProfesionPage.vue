...
onMounted(async () => {
    try {
        ...
        const profesion = await profesionesService.getById(id);
        if (profesion) {
            formData.value.nombre = profesion.nombre;
        }
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'Error cargando profesión' });
...
const handleSubmit = async () => {
    ...
    try {
        loading.value = true;
        await profesionesService.update(parseInt(props.id), formData.value);
        $q.notify({ type: 'positive', message: 'Profesión actualizada correctamente' });
        router.push('/profesiones');
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'No se pudo actualizar la profesión' });
    } finally {
        loading.value = false;
    }
};
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-8 col-lg-6">
        <div class="row items-center q-mb-lg">
             ...
             <div>
                <div class="text-h5 text-weight-bold text-primary">Editar Profesión</div>
             </div>
        </div>
