...
import type { CreateTipoIdentDto } from '../types/tipo-ident';
...
const formData = ref<CreateTipoIdentDto>({
  codigo: 0,
  nombre: '',
  abreviado: '',
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await tipoIdentService.create(formData.value);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al crear el tipo de identificación';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Nuevo Tipo de Identificación</h1>
            <p class="form-subtitle">Complete la información del tipo de identificación</p>
          </div>
        </div>
      </div>
