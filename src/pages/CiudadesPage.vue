...
const loadData = async () => {
  try {
    ...
    const data = await ciudadService.getByEmpresa(empresaId);
    ciudades.value = data || [];
  } catch (error) {
...
const filteredData = () => {
  if (!filter.value) return ciudades.value;
  return ciudades.value.filter(item =>
    item.nombre.toLowerCase().includes(filter.value.toLowerCase()) ||
    item.codigo.toLowerCase().includes(filter.value.toLowerCase())
  );
};

const handleNew = () => {
  router.push('/ciudades/new');
};

const handleEdit = (ciudad: Ciudad) => {
  router.push(`/ciudades/edit/${ciudad.codigo}`);
};
...
const handleDelete = async (ciudad: Ciudad) => {
  $q.dialog({
    ...
    try {
      await ciudadService.delete(ciudad.codigo);
      $q.notify({
        type: 'positive',
...
const handleExportExcel = () => {
  try {
    ...
    exportToExcel(filteredData(), exportColumns, 'ciudades');
...
const handleExportPDF = () => {
  try {
    ...
    exportToPDF(filteredData(), exportColumns, 'ciudades', 'Listado de Ciudades');
      ...
      <div class="page-header">
        <div class="header-content">
          <div class="title-section">
            <q-icon name="location_city" size="32px" color="primary" class="q-mr-sm" />
            <div>
              <h1 class="page-title">Ciudades</h1>
              <p class="page-subtitle">Gestión de ciudades</p>
            </div>
          </div>
        ...
        >
          ...
          <template v-slot:body-cell-nombre="props">
            <q-td :props="props">
              <div class="flex items-center">
                <q-icon name="location_city" size="20px" color="grey-6" class="q-mr-sm" />
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center q-gutter-md q-py-xl">
              <q-icon name="location_city" size="64px" color="grey-4" />
              <div class="text-center">
                <div class="text-h6 text-grey-6">No hay ciudades registradas</div>
                <div class="text-caption text-grey-5">Comienza agregando una nueva ciudad</div>
              </div>
            </div>
          </template>
        </q-table>
