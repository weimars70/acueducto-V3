...
import type { TipoMovimientoItem } from '../types/tipo-movimiento-item';

const $q = useQuasar();
const router = useRouter();
const authStore = useAuthStore();
const items = ref<TipoMovimientoItem[]>([]);
...
const loadData = async () => {
  try {
    ...
    const data = await tipoMovimientoItemService.getByEmpresa(empresaId);
    items.value = data || [];
  } catch (error) {
    console.error('Error al cargar tipos movimiento item:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los tipos de movimiento item'
    });
    items.value = [];
  } finally {
    loading.value = false;
...
const handleNew = () => {
  router.push('/tipo-movimiento-item/new');
};

const handleEdit = (item: TipoMovimientoItem) => {
  router.push(`/tipo-movimiento-item/edit/${item.id}`);
};

const handleDelete = async (item: TipoMovimientoItem) => {
  $q.dialog({
    title: 'Confirmar',
    ...
    try {
      await tipoMovimientoItemService.delete(item.id);
      $q.notify({
        type: 'positive',
      ...
      <div class="page-header">
        <div class="header-content">
          <div class="title-section">
            <q-icon name="swap_horiz" size="32px" color="primary" class="q-mr-sm" />
            <div>
              <h1 class="page-title">Tipo Movimiento Item</h1>
              <p class="page-subtitle">Gestión de tipos de movimiento de items</p>
            </div>
          </div>
          <q-btn
            unelevated
          ...
          <template v-slot:body-cell-nombre="props">
            <q-td :props="props">
              <div class="flex items-center">
                <q-icon name="swap_horiz" size="20px" color="grey-6" class="q-mr-sm" />
          ...
          <template v-slot:no-data>
            <div class="full-width row flex-center q-gutter-md q-py-xl">
              ...
              <div class="text-center">
                <div class="text-h6 text-grey-6">No hay tipos registrados</div>
