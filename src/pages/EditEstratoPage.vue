...
const loadData = async () => {
    try {
        ...
        const data = await estratosService.getById(id);
        formData.value = {
            nombre: data.nombre,
...
const handleSubmit = async () => {
    ...
    try {
        loading.value = true;
        await estratosService.update(Number(route.params.id), formData.value);
        $q.notify({ type: 'positive', message: 'Estrato actualizado correctamente' });
        router.push('/estratos');
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'No se pudo actualizar el estrato' });
    } finally {
        loading.value = false;
    }
};
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-8 col-lg-6">
        <div class="row items-center q-mb-lg">
             ...
             <div>
                <div class="text-h5 text-weight-bold text-primary">Editar Estrato</div>
             </div>
        </div>
