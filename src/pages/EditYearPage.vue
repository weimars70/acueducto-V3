...
import type { UpdateYearsDto } from '../types/years';
...
const yearParam = ref<number>(parseInt(route.params.year as string));

const formData = ref<UpdateYearsDto>({
  year: 0,
...
const loadYear = async () => {
  try {
    ...
    const yearData = await yearsService.getByYear(yearParam.value);
    formData.value = {
      year: yearData.year,
      empresa_id: yearData.empresa_id
    };
  } catch (error) {
    console.error('Error al cargar año:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los datos del año'
    });
    router.push('/years');
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await yearsService.update(yearParam.value, dataToUpdate);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al actualizar el año';
    $q.notify({
      type: 'negative',
      message
    });
    ...
    <div class="form-container">
      <div class="form-header">
        ...
        <div class="header-content">
          <q-icon name="calendar_today" size="40px" color="primary" />
          <div class="header-text">
            <h1 class="form-title">Editar Año</h1>
            <p class="form-subtitle">Modifique la información del año fiscal</p>
          </div>
        </div>
      </div>
            ...
            <div class="row q-col-gutter-md">
              <div class="col-12 col-md-4">
                <div class="input-wrapper">
                  <label class="input-label">Año <span class="required">*</span></label>
                  <q-input
                    ...
                    :rules="[
                      ...
                      val => val > 1900 && val < 3000 || 'Año inválido'
                    ]"
                    class="modern-input"
                    disable
                  >
