...
const handleSubmit = async () => {
    ...
    try {
        loading.value = true;
        await sectoresService.create(formData.value);
        $q.notify({ type: 'positive', message: 'Sector guardado correctamente' });
        router.push('/sectores');
    } catch (error) {
        console.error(error);
        $q.notify({ type: 'negative', message: 'No se pudo guardar el sector' });
    } finally {
        loading.value = false;
    }
};
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-8 col-lg-6">
        <div class="row items-center q-mb-lg">
             ...
             <div>
                <div class="text-h5 text-weight-bold text-primary">Nuevo Sector</div>
             </div>
        </div>
