...
import type { UpdateTipoIdentDto } from '../types/tipo-ident';
...
const formData = ref<UpdateTipoIdentDto>({
  codigo: 0,
  nombre: '',
  abreviado: '',
...
const loadTipoIdent = async () => {
  try {
    loadingData.value = true;
    const tipoIdent = await tipoIdentService.getByCodigo(codigo.value);
    formData.value = {
      codigo: tipoIdent.codigo,
      nombre: tipoIdent.nombre,
      abreviado: tipoIdent.abreviado,
      empresa_id: tipoIdent.empresa_id
    };
  } catch (error) {
    console.error('Error al cargar tipo de identificación:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al cargar los datos del tipo de identificación'
    });
    router.push('/tipo-ident');
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await tipoIdentService.update(codigo.value, dataToUpdate);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al actualizar el tipo de identificación';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Editar Tipo de Identificación</h1>
            <p class="form-subtitle">Modifique la información del tipo de identificación</p>
          </div>
        </div>
      </div>
        ...
        <q-form @submit="handleSubmit" class="form-content">
          <div class="form-section">
            ...
            <div class="row q-col-gutter-md">
              <div class="col-12">
                <div class="input-wrapper">
                  <label class="input-label">Abreviado</label>
                  <q-input
