...
import type { CreateConceptoNominaDto } from '../types/concepto-nomina';

const $q = useQuasar();
const router = useRouter();
const loading = ref(false);

const formData = ref<CreateConceptoNominaDto>({
  codigo: '',
  nombre: '',
  descripcion: '',
  tipo: 'DEVENGADO',
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true;
    await conceptoNominaService.create(formData.value);
    $q.notify({
      type: 'positive',
    ...
    const message = error?.response?.data?.message || error?.message || 'Error al crear el concepto de nómina';
    $q.notify({
      type: 'negative',
      message
    });
      ...
      <div class="form-header">
        ...
        <div class="header-content">
          ...
          <div class="header-text">
            <h1 class="form-title">Nuevo Concepto de Nómina</h1>
            <p class="form-subtitle">Complete la información del concepto</p>
          </div>
        </div>
      </div>
