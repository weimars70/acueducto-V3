...
void ManageTrailing(string sym)
{
   ...
   double openPrice, currentSL, point, currentPrice, profitPips;
   double newSL, candidate;
   ...
   for(i = 0; i < posTotal; i++)
   {
      if(PositionSelectByIndex(i))
      {
         ...
         currentPrice = (ptype == POSITION_TYPE_BUY) ?
                        SymbolInfoDouble(sym, SYMBOL_BID) :
                        SymbolInfoDouble(sym, SYMBOL_ASK);

         profitPips = ((ptype == POSITION_TYPE_BUY) ?
